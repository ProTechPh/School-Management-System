...
export default function TeacherSchedulePage() {
  ...
  const fetchData = async () => {
    ...
    const { data } = await supabase
      ...
      .select(`
        ...
        class:classes!inner (name, subject, teacher_id)
      `)
  ...
  if (loading) {
    return (
      <div className="min-h-screen">
        <DashboardHeader title="My Schedule" subtitle="View your weekly teaching schedule" />
  ...
  return (
    <div className="min-h-screen">
      <DashboardHeader title="My Schedule" subtitle="View your weekly teaching schedule" userId={userId} />
            ...
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <div className="rounded-lg border border-border p-4 text-center">
                <p className="text-2xl font-bold text-card-foreground">
                  {[...new Set(schedule.map((s) => s.class_name))].length}
                </p>
                <p className="text-sm text-muted-foreground">Unique Classes</p>
              </div>
              <div className="rounded-lg border border-border p-4 text-center">
                <p className="text-2xl font-bold text-card-foreground">
                  {[...new Set(schedule.map((s) => s.room).filter(Boolean))].length}
                </p>
