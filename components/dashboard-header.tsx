...
export function DashboardHeader({ title, subtitle, description, role, userId = "" }: DashboardHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 lg:h-16 items-center justify-between border-b border-border bg-background/95 px-4 lg:px-6 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="min-w-0 flex-1">
        ...
        {displaySubtitle && <p className="text-xs lg:text-sm text-muted-foreground truncate">{displaySubtitle}</p>}
      </div>
